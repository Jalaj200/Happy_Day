...
# ──────────────────────────────────────────────
# 2. Gallery Category Model
# ──────────────────────────────────────────────
class GalleryCategory(models.Model):
    """
    Category for organizing gallery images (e.g., 'Our Dates', 'Travel', 'Celebrations').
    Acts as the target for the GalleryImage.category ForeignKey.
    """

    name = models.CharField(
        max_length=100,
        help_text="Category name displayed on the filter buttons.",
    )
    slug = models.SlugField(
        unique=True,
        help_text="URL-friendly identifier (e.g., 'our-dates').",
    )
    icon = models.CharField(
        max_length=100,
        default="fa-solid fa-heart",
        blank=True,
        help_text="Font Awesome icon class for the category filter button.",
    )
    order = models.PositiveIntegerField(
        default=0,
        help_text="Display order for category buttons.",
    )
    is_active = models.BooleanField(
        default=True,
        help_text="Only active categories are shown in filters.",
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
# ──────────────────────────────────────────────
# 3. Gallery Image Model (with Foreign Keys)
# ──────────────────────────────────────────────
class GalleryImage(models.Model):
    """
    A single photo item in the premium masonry photo gallery.

    Relationships:
        1. category (ForeignKey -> GalleryCategory): One-to-Many relationship.
           Organizes photos into selectable filter tabs. Uses on_delete=SET_NULL
           so that if a category is deleted, photos remain in the gallery uncategorized.
        2. memory (ForeignKey -> Memory): One-to-Many relationship.
           Links a photo directly to a milestone Memory on the timeline, creating a
           rich relational bridge between the timeline and the gallery!
    """

    title = models.CharField(
        max_length=200,
        help_text="Title of the photo (shown in lightbox and on hover).",
    )
    image = models.ImageField(
        upload_to="gallery/%Y/%m/",
        help_text="Main photo upload (stored cleanly by year/month in media/gallery/).",
    )
    thumbnail = models.ImageField(
        upload_to="gallery/thumbnails/",
        blank=True,
        null=True,
        help_text="Optional smaller thumbnail for faster masonry grid loading.",
    )
    category = models.ForeignKey(
        ...
        blank=True,
        related_name="images",
        help_text="Select a category for filtering (e.g., 'Our Dates').",
    )
    memory = models.ForeignKey(
        ...
        help_text="Optional: Associate this photo with a specific milestone Memory.",
    )
    caption = models.TextField(
        blank=True,
        help_text="Romantic caption displayed inside the full-screen lightbox.",
    )
    date_taken = models.DateField(
        blank=True,
        null=True,
        help_text="Calendar date when this photo was taken.",
    )
    is_featured = models.BooleanField(
        default=False,
        help_text="Featured images are displayed larger in the masonry layout.",
    )
    is_active = models.BooleanField(
        default=True,
        help_text="Only active photos are displayed on the website.",
    )
    order = models.PositiveIntegerField(
        default=0,
    ...
    def display_image(self):
        """Return thumbnail if available, otherwise the main image."""
        if self.thumbnail:
